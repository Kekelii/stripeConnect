...
export async function createWallet(userId, ip) {
    //get user details
    try {
        ...
        //create wallet
        const createdWallet = await userWallet.createUserWallet(userId, customer, connectedAccount);
        return {
            error: null,
...
export async function removeWallat(userId) {
    const userWallet = await wallet.findOne({ userId });
    if (!userWallet) {
        throw new AppError('wallet does not exist', 404);
    }
    try {
        const removeConnectedAcount = await stripe.disconnectAccount(userWallet.connectedAccountId);
        const removeCustomerAccount = await stripe.removeCustomer(userWallet.customerId);
...
export async function topUpWallet(userId, amount) {
    try {
        ...
        const paymentIntent = await stripe.createPaymentIntent(userId, walletExists.connectedAccountId, amount);
        if (!paymentIntent) {
...
//hook event
export async function transferCreated(transferObject) {
    try {
        const originAccount = transferObject.metadata.originAccount ||
            transferObject.metadata.destinationAccount;
        const balanceOfOrigin = await stripe.checkAccountBalance(originAccount);
        const update = await strip_Account_to_Wallet.updateBalance(balanceOfOrigin, originAccount);
        if (transferObject.metadata.destinationAccount) {
            await WalletToWalletTranscations.findOneAndUpdate({ _id: transferObject.id }, { status: 'successfull' });
        }
    }
    catch (error) {
...
export async function createExternalBankaccount(userId, bankDetails) {
    const _wallet = await wallet.findOne({ userId });
    if (!_wallet) {
        throw new AppError('user account does not exist', 404);
    }
    const connectAccountId = _wallet.connectedAccountId;
    const details = {
        external_account: {
            object: 'bank_account',
...
export async function payoutMade(payoutObject) {
    try {
        ...
        await Withdrawals.findOneAndUpdate({ _id: payoutObject.id }, { status: 'succesful' });
    }
    catch (err) {
        logger.warn(err, 'something went wrong');
    }
}
export async function allBankAccounts(userId) {
    try {
        const walletObj = await wallet.findOne({ userId });
        const connectedAccount = walletObj?.connectedAccountId;
...
export async function balance(userId) {
    try {
        ...
        const availableBalance = formatBalance(balance.available.reduce((sum, b) => sum + b.amount, 0));
        const pendingBalance = formatBalance(balance.pending.reduce((sum, b) => sum + b.amount, 0));
        return {
            availableBalance,
            pendingBalance,
        };
...
export async function transactions(transactionType, start, limit, userId) {
    const _page = start || 1;
    ...
    try {
        ...
        if (transactionType === 'WalletToWallet') {
            ...
            const userIds = Array.from(new Set(data.flatMap((tx) => [
...
export async function downloadTransaction(transactionType, month, year, userId) {
    try {
        const _wallet = await wallet.findOne({ userId });
...
export async function graphData(userId) {
    try {
...
export async function notifictaions(userId, start, limit) {
    const _page = start || 1;
    const _limit = limit || 10;
    const _skip = (_page - 1) * limit;
    ...
    const returnedData = nots.map((i) => {
        return {
            ...
            details: {
                ...
                senderName: `${dic[i.details.senderId]?.firstName} ${dic[i.details.senderId]?.lastName}`,
            },
        };
    });
    return returnedData;
