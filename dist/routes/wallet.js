...
import { allBankAccounts, balance, createExternalBankaccount, createWallet, downloadTransaction, getWallet, graphData, notifictaions, removeWallat, topUpWallet, transactions, } from '../controllers/wallet/main.js';
...
const _Wallet = express.Router();
//get userDetails
_Wallet.get('/user', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const user = await userWallet.getUser(userId);
    res.json(user);
}));
// get wallet details
_Wallet.get('/walletInfo', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const wallet = await getWallet(userId);
    res.json(wallet);
}));
//create wallet or returns a wallet that already exists
_Wallet.get('/create/:userId', asyncHandler(async (req, res) => {
    const { userId } = req.params;
    const Ternate = await createWallet(userId, req.ip);
    res.send(Ternate);
}));
//delete wallet
_Wallet.delete('/delete/:userId', asyncHandler(async (req, res) => {
    const { userId } = req.params;
    const deletedWallet = await userWallet.deleteWallet(userId);
    res.json({ data: deletedWallet.data });
}));
//update Wallet
_Wallet.post('/update/:userId', asyncHandler(async (req, res) => {
    const { userId } = req.params;
    const data = req.body;
...
//wallet topUp
_Wallet.post('/topup', asyncHandler(async (req, res) => {
...
//delete wallet
_Wallet.delete('/deleteAccount/:userId', asyncHandler(async (req, res) => {
    const { userId } = req.params;
    const removedConfirmation = await removeWallat(userId);
    res.json(removedConfirmation);
}));
//wallet to wallet transfer
_Wallet.post('/walletToWallet', asyncHandler(async (req, res) => {
    const userId = req.userId;
...
_Wallet.post('/addExternalBank', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const { accountNumber, country, currency, accountHolderName, routingNumber, } = req.body;
...
_Wallet.post('/payout', asyncHandler(async (req, res) => {
    const userId = req.userId;
...
_Wallet.get('/bankaccounts', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const bankAccounts = await allBankAccounts(userId);
    res.send(bankAccounts);
}));
//get wallet balance
_Wallet.get('/balance', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const _balance = await balance(userId);
    res.json(_balance);
}));
//get transcation hsitory
_Wallet.get('/transactions', asyncHandler(async (req, res) => {
    const userId = req.userId;
...
_Wallet.get('/transactions/download', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const { type, month, year } = req.query;
    const transactions = await downloadTransaction(type, month, year, userId);
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="${type}-${year || 'all'}-${month || 'all'}.csv"`);
    res.send(transactions);
}));
//get graph data
_Wallet.get('/graphdata', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const data = await graphData(userId);
    res.send(data);
}));
//get notifications
_Wallet.get('/notifications', asyncHandler(async (req, res) => {
    const userId = req.userId;
