...
export async function processPayout(
	...
	try {
		const walletObj = (await wallet.findOne({ userId })) as Wallet;
		const connectedAccountId = walletObj.connectedAccountId as string;
		const connectedAccounts = walletObj.connectedBankAccounts as Array<string>;
		...
		const externalBankDetails = stripe.getExternalBankaccountDetails(
			connectedAccountId,
		...
		const payout = await stripe.initiatePayout(
			connectedAccountId,
			selectedCurrency,
			externalBankAccountId,
			amount
...
//hook event
export async function addExternalBankAccountToWallet(
	createdExternalBankAccount: Stripe.ExternalAccount
) {
	try {
