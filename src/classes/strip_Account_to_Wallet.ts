...
class AccountTOWallet {
	...
	//hooks event
	async updatePaymentIntent(StripeEventObject: Stripe.PaymentIntent) {
		const dataObject = StripeEventObject as Stripe.PaymentIntent;
		const paymentIntent = dataObject.id;
		const paymentIntentStatus = dataObject.status;
		const metadata = dataObject.metadata;
		const connectedAccountId = metadata.accountId as string;
		...
		try {
			...
			if (paymentIntentStatus === 'succeeded') {
				const balance: number = (await stripe.connectedAccountBalance(
					connectedAccountId
				)) as number;
				await wallet.updateOne(
					{ userId },
					{ $set: { pendingBalance: balance / 100 } }
				);
			}
		} catch (error) {
	...
	//hook event
	async updateBalance(balance: Stripe.Balance, connectedAccountId: string) {
		try {
			if (!balance || !connectedAccountId) {
				if (!balance) {
					throw new AppError(
						'Balance object is required for  updating balance',
						500
					);
				} else {
			...
			const availableAmount = _balance.available.reduce(
				(sum, b) => sum + (b.amount ?? 0),
				0
			);
			const pendingAmount = _balance.pending.reduce(
