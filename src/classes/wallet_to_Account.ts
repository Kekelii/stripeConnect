...
class walletToAccount {
	...
	async createConnectedAccount(
		...
		try {
			...
			return { data: account.id as unknown as string, error: null };
		} catch (error: unknown) {
			const errorMessage =
	...
	async createAndAddbankaccount(
		...
		try {
			...
			const createdAndAddbankaccount =
				await this.initializedStrip.accounts.createExternalAccount(
					_wallet.connectedAccountId,
					{
						external_account: {
							object: 'bank_account',
							country: 'CZ',
			...
			await wallet.findOneAndUpdate(
				{ userId },
				{ $push: { connectedBankAccounts: createdAndAddbankaccount } }
			);
	...
	async initiateBankTransfer(
		...
		const banks: Stripe.BankAccount[] = bankDetails.connectedBankAccounts;
		if (banks.length < 1) {
			throw new AppError('No bank account linked with wallet', 404);
		}

		const acceptedBank: Stripe.BankAccount | undefined = banks.find(
			(b: any) => b.id === destinationId
		);
		if (!acceptedBank) {
		...
		const transfer = await this.initializedStrip?.transfers.create({
			...
			destination: acceptedBank.account as string, // Connected account ID
	...
	async disconnectAllaccount(id: string): Promise<Result<boolean>> {
		try {
