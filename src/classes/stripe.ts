...
const webHookSecret = process.env.WEBHOOKS_SECRETS as string;
class _Stripe extends Stripe {
	...
	//create customer
	async createCustomer(customerPayload: StripeCustomerPayload) {
	...
	async createTernatePlus(customerId: string, currency?: string) {
	...
	async getSubscriptionsOfCustomer(customerId: string) {
	...
	async createConnectedAccount(
		Payload: StripeConnectedAccountUser,
		ip: string
	) {
	...
	//disconnect Account
	async disconnectAccount(connectedAccountId: string) {
	...
	async addExternalaccount(
		...
		const externalAccount = await this.accounts.createExternalAccount(
			connectedAccountId,
			{
				external_account: {
					object: 'bank_account',
	...
	async getExternalAccounts(accountId: string) {
	...
	async updatePaymentIntent(
		...
		const updatedIntent = await this.paymentIntents.update(
			paymentIntentId,
			updates
		);
	...
	async connectedAccountBalance(accountId: string) {
	...
	async transferMoneyFromConnectedAccount(
		currency: string,
		amount: number,
	...
	async validateWebHook(sig: string, body: Buffer | string) {
		return this.webhooks.constructEvent(body, sig, webHookSecret);
	}
	//add externalBank external to account
	async addBankAccount(
		connectedAccountId: string,
		bankDetails: Stripe.AccountCreateExternalAccountParams
	) {
		const bank = await this.accounts.createExternalAccount(
			connectedAccountId,
			bankDetails
		);
		return bank;
	...
	async getExternalBankaccountDetails(
		connectAccountId: string,
		externalBankId: string
	) {
		return this.accounts.retrieveExternalAccount(
			connectAccountId,
			externalBankId
		);
	}
	...
	async getBankAccounts(connectAccountId: string) {
