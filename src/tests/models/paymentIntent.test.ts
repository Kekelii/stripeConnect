...
const MONGODB_TEST_URI =
	process.env.MONGODB_TEST_URI || 'mongodb://localhost:27017/depositoo_test';
...
describe('PaymentIntents Model Tests', () => {
	...
	test('should update payment intent status', async () => {
		...
		// Update status
		const updated = await PaymentIntents.findByIdAndUpdate(
			created._id,
			{ status: 'succeeded' },
			{ new: true }
		);
	...
	test('should handle paymentIntentId as object', async () => {
		...
		assert.strictEqual(
			...
			complexPaymentIntent.amount
		);
		assert.strictEqual(
			result.paymentIntent.currency,
			complexPaymentIntent.currency
		);
	});
