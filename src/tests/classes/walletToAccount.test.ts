...
import walletToAccount from '../../classes/wallet_to_Account.js';
...
describe('walletToAccount', () => {
	...
	it('should return account id when creation succeeds', async () => {
		...
		const result = await walletToAccount.createConnectedAccount(
			'68b841954c3828ced84ccfb0',
			'127.0.0.1'
		);
		console.log(result);
	...
	it.skip('should handle uninitialized Stripe', async () => {
		(walletToAccount as any).initializedStrip = undefined;
	...
	it.skip('should throw AppError if user not found', async () => {
		...
		await assert.rejects(
			walletToAccount.createConnectedAccount('invalidUserId', '127.0.0.1'),
			(err: any) => {
